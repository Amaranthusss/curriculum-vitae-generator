...
export const useNameLabel = () => {
	...
	const fullNameMarginBottom: number = useProfileStore(
		({ fullNameMarginBottom }) => fullNameMarginBottom
	);
	...
	const renderNameLabel = useCallback((): Content[] => {
		return [
			...
			},
			{
				...
				marginBottom: fullNameMarginBottom,
				layout: "noBorders",
			},
		];
	}, [joinNameAndSurname, primaryColor, primaryBgColor, fullNameMarginBottom]);
